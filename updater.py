...
1. Download the asset ``TW1DungeonEditor.exe`` of that release next to the
   running exe as ``TW1DungeonEditor.exe.new``.
...
REPO = 'MedievalDev/TW1_DungeonEditor'
API_LATEST = f'https://api.github.com/repos/{REPO}/releases/latest'
LATEST_PAGE = f'https://github.com/{REPO}/releases/latest'
ASSET = 'TW1DungeonEditor.exe'
TIMEOUT = 6


def parse_version(text):
    """'v3.3.2' / '3.3.2' -> (3, 3, 2); None when it is not a version."""
...
def release_info(payload):
    """What the tool needs from a GitHub release JSON: {'version', 'tag',
    'page', 'notes', 'url', 'size', 'sha256'} (url/sha256 None when the
    release has no exe asset or no digest)."""
...
def fetch_latest(timeout=TIMEOUT):
    ...
    req = urllib.request.Request(API_LATEST, headers={
        ...
        'User-Agent': f'TW1DungeonEditor/{VERSION}'})
...
def start_swap(exe, new, pid=None):
    ...
    fd, path = tempfile.mkstemp(prefix='tw1de_update_', suffix='.bat')
